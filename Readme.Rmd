...
Here is the distribution of the various descriptions for trance and possession in our training sample.
...
trance %>% 
  select(description,code,name,possession_trance,count) %>%
  rename(outcome = possession_trance) %>% 
  add_row(name = 'Total', description = '', count = 151) %>% 
  kable('simple')
```

We see that only about 10% of societies have no trance or possession phenomena. From the rest, a majority have possession trance in some form. Following @bourguignon1968 we can reduce this to a binary: 81 societies have possession trance, 70 have other forms of trance and possessior or show neither.
